"""
Persona construction utilities.

This file answers the question "who are the people in the network?"
It can sample demographics programmatically and, when requested, use an LLM
to make those personas feel more human by adding names or interests.
"""
...
def generate_persona(seed, sorted_triplets, cdf):
    ...
    """
    # Using the seed here makes persona generation reproducible.
    np.random.seed(seed)
    person = {}
...
def generate_names(personas, demos, model, verbose=False):
    ...
    """
    # Each persona is prompted independently so the model can tailor the name
    # to that person's demographics.
    for nr in personas:
...
def generate_interests(personas, demos, model, verbose=False):
    ...
    """
    # Demographic order is shuffled so the prompt is less rigid and repetitive.
    for nr in personas:
...
def parse_reason(model, reason, demos_to_include, verbose=False):
    ...
    """
    # This turns unstructured model explanations back into structured labels.
