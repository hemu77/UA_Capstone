"""
Plotting helpers for graphs and metric tables.

This file is the visualization layer. It does not decide friendships and it does
not compute the main research metrics; it only turns already-generated data into
PNG figures that are easier for humans to inspect.
"""
...
PATH_TO_TEXT_FILES = './text-files'
GRAPH_TYPES = ['real', 'global', 'local', 'sequential'] #  'iterative']
SHOW_PLOTS = False

def parse_save_name(save_name):
    # Experiment names encode method, model, and optional suffixes in one string.
...
def make_plot(network_metrics_df, save_name=None, plot_type='default', plot_homophily=False, homophily_metric='same_ratio',
    ...
    if palette is None:
        palette = get_pallete(network_metrics_df)
    # Default style shows both individual observations and a simple summary.
    if plot_type == 'default':
...
def plot_edges(num_edges, save_name):
    # This is most informative when a condition has multiple seeds / graphs.
